...
class upLoadPage(Page):
   def __init__(self, *args, **kwargs):
       ...
       def browseImg():
           ...
           global bindata
           root.filename =  filedialog.askopenfilename(initialdir = "/home/pi/Pictures/toSell/",title = "Select Image",filetypes = (("jpeg","*.jpg"),("All files","*.*")))
       ...
       def upLoadAll():
           if (varClass.get()==''):
               ...
               def cash(price):
                   croot = Tk()
                   kroot = Tk()
                   frame = Frame(croot, width=200, height=200)
                   croot.geometry("150x150+0+0")
                   croot.wm_attributes('-type', 'splash')
                   frame.grid(row=0, column=0, sticky="NESW")
                   frame.grid_rowconfigure(0, weight=1)
                   frame.grid_columnconfigure(0, weight=1)
                   croot.grid_rowconfigure(0, weight=1)
                   croot.grid_columnconfigure(0, weight=1)
                   lblScan = Label(croot, text = 'Tap to pay \n $'+str(price))
                   lblScan.grid(row=0, column=0)
                   croot.after(2000, croot.destroy)
                   kroot.geometry("150x150+0+0")
                   kroot.wm_attributes('-type', 'splash')
                   kroot.grid_rowconfigure(0, weight=1)
                   kroot.grid_columnconfigure(0, weight=1)
                   lblScann = Label(kroot, text = 'Payment Succesful \n Ad Uploaded')
                   lblScann.grid(row=0, column=0)
                   kroot.after(4000, kroot.destroy)
       ...
       brwsImgBtn = Button(self, text = "Image", command = browseImg, height=4, width=10)#browsImg
...
class RegPage(Page):
   def __init__(self, *args, **kwargs):
       ...
       passTb = Entry(self, show="*")
       passTb.grid(row=2, column=2, columnspan=6)
       confirmPassTb = Entry(self, show="*")
...
class ChangePassPage(Page):
    def __init__(self, *args, **kwargs):
       ...
       pass1 = Entry(self, show="*")
       pass1.grid(column = 5, row=3, columnspan = 4)
       pass1.insert(0, '00000000')
       pass2 = Entry(self, show="*")
       pass2.grid(column = 5, row=4, columnspan = 4)
       pass2.insert(0, '00000000')
       row=''
